...
def sort_pieces_min_waste_hybrid(pieces, slabs):
    def fit_stats(piece):
        ...
        for sw, sh in slabs:
            ...
            for dims in [(pw, ph), (ph, pw)]:
                if dims[0] <= sw and dims[1] <= sh:
                    fitting_slabs += 1
                    waste_ratio = (slab_area - (dims[0] * dims[1])) / slab_area
                    best_waste_ratio = min(best_waste_ratio, waste_ratio)
        return fitting_slabs, best_waste_ratio

    # Sort by fewest fitting slabs, then lowest waste ratio, then largest area
    return sorted(
        pieces,
        key=lambda p: (fit_stats(p)[0], fit_stats(p)[1], -(p[1] * p[2]))
    )
...
def nest_pieces_guillotine(required_pieces: List[Tuple[str, float, float]], available_slabs: List[Tuple[float, float]], use_smart_combo: bool = True, granite_mode: bool = False):
    ...
    if granite_mode:
        ...
        pieces = sort_pieces_min_waste_hybrid(required_pieces, unique_slabs)
        ...
        # Quartz Mode - Infinite supply of each available slab size
        def try_infinite_supply(required_pieces, slabs):
            ...
            while pieces:
                ...
                # Try each slab size to see which gives minimal leftover waste
                for sw, sh in slabs:
                    if sh > sw:
                        sw, sh = sh, sw
                    ...
                    # Pick slab with least leftovers, then least wastage
                ...
                if not best_slab:  # No fit found
                    break
    
                results.append((best_slab, best_layout))
                used_slabs.append(best_slab)
                pieces = best_leftovers
